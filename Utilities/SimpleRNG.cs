...
{
    public class SimpleRNGCore
    {
        private uint m_w;
        private uint m_z;

        public uint W { get { return m_w; } set { m_w = value; } }
        public uint Z { get { return m_z; } set { m_z = value; } }

        public SimpleRNGCore()
        {
        ...
        public float Range(float from, float to)
        {
            ...
            float range = to - from;
            float u = (float)(GetUniform() * range);

            return from + u;
        }

        public int EvenlyDistribute(int exclusiveMax)
        {
            uint n = 0xFFFFFFFF - (0xFFFFFFFF % (uint)exclusiveMax);
            uint u = 0;
            do
            {
                u = GetUint();
    ...
    {
        return msRamdomCore.Range(from, to);
    }
